...
# Configuration
$ScriptPath = "src/glue_job/main.py"
...
function Show-Usage {
    ...
    Write-Host "  - Glue job script to s3://bucket/src/glue_job/main.py"
...
aws s3 cp $ScriptPath "s3://$BucketName/src/glue_job/main.py"
if ($LASTEXITCODE -eq 0) {
    Write-Success "Uploaded Glue job script to s3://$BucketName/src/glue_job/main.py"
} else {
